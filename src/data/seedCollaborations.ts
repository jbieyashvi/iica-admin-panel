...
export function buildCollaborations(users: UserRecord[], portfolios: PortfolioRecord[], now: number): CollaborationRecord[] {
  ...
  for (const s of SPECS) {
    ...
    const record: CollaborationRecord = {
      ...
      intent: s.intent,
      matchSource: Number(s.id.replace('col_', '')) % 2 === 0 ? 'natural_language' : 'direct_profile',
      naturalLanguageRequirement: `I'm looking for a ${invited.primaryCategory} to work with on ${INTENT_LABEL[s.intent].toLowerCase()} — ${FORMAT_LABEL[s.format].toLowerCase()}, ideally around ${s.location}.`,
      extractedRequirement: `${INTENT_LABEL[s.intent]} · ${invited.primaryCategory} · ${FORMAT_LABEL[s.format]} · ${s.location}`,
