...
const SPECS: Spec[] = [
  ...
  { id: 'prod_sprint', sellerId: 'usr_abhishek', type: 'masterclass', category: 'Sports Coaching', title: 'Sprint Training Masterclass', price: 999, status: 'awaiting_review', capacity: 25, seats: 0 },
  { id: 'prod_artprint', sellerId: 'usr_sophia', type: 'physical', category: 'Collectibles', title: 'Limited Edition Art Print', price: 3500, status: 'awaiting_review', stock: 20 },
  { id: 'prod_modeltemplates', sellerId: 'usr_kabir', type: 'digital', category: 'Templates & Resources', title: 'Modeling Portfolio Templates', price: 299, status: 'changes_requested' },
  ...
  { id: 'prod_branding', sellerId: 'usr_devang', type: 'masterclass', category: 'Personal Branding', title: 'Personal Branding Intensive', price: 3999, status: 'changes_requested', capacity: 20, seats: 0 },
...
function buildTimeline(id: string, status: ProductStatus, base: number): TimelineEvent[] {
  ...
  const submitted = ['awaiting_review', 'changes_requested', 'published', 'out_of_stock', 'hidden', 'archived'];
  if (submitted.includes(status)) t.push({ id: `${id}_tl1`, key: 'submitted', label: 'Submitted for review', at: new Date(base + 2 * DAY).toISOString() });
  if (status === 'changes_requested') t.push({ id: `${id}_tl2`, key: 'changes', label: 'Changes requested by admin', at: new Date(base + 3 * DAY).toISOString() });
